...
template/agentic-workflow-gui                              ← template branch (never modified directly)
  └── master_{task_name}                                   ← task branch (one per user request)
        ...
        ├── copilot/architect-{task_name}-{sessionID}         ← Architect's work → merges back to task branch
        ├── copilot/it-{task_name}-{sessionID}                ← IT's work (setup) → merges back to task branch
        ├── copilot/developer-{task_name}-{sessionID}         ← Developer's work → merges back to task branch
        ├── copilot/tester-{task_name}-{sessionID}            ← Tester's work → merges back to task branch
        └── copilot/it-release-{task_name}-{sessionID}        ← IT's work (release) → merges back to task branch
```
...
When the user declines a PR and asks to hand over directly, you MUST merge your agent branch into the task branch first. This ensures the next agent has all previous work:

```bash
git checkout master_{task_name}
git merge copilot/{agent}-{task_name}-{sessionID}
...
```bash
git checkout master_{task_name}
git checkout -b copilot/product-owner-{task_name}-{sessionID}
```
...
```bash
git checkout master_{task_name}
git checkout -b copilot/cost-analyst-{task_name}-{sessionID}
```
...
```bash
git checkout master_{task_name}
git checkout -b copilot/architect-{task_name}-{sessionID}
```
...
```bash
git checkout master_{task_name}
git checkout -b copilot/it-{task_name}-{sessionID}
```
...
```bash
git checkout master_{task_name}
git checkout -b copilot/developer-{task_name}-{sessionID}
```
...
```bash
git checkout master_{task_name}
git checkout -b copilot/tester-{task_name}-{sessionID}
```
...
```bash
git checkout master_{task_name}
git checkout -b copilot/it-release-{task_name}-{sessionID}
```
